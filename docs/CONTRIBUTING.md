...
| Testing / Validation / Reproducibility (self-tested) | ✅ | Each team member has tested and validated their own code. We also have a `nb_validate.py` script that is manually run with a report on each notebook's *validated* status written to `.artifacts_ci/validation/`. | 1) Console output for successful NB validation:![alt text](.alt-text-images-for-docs/.contributing/bp15_ex1.jpg)</br></br>2) Local validation log file:![alt text](.alt-text-images-for-docs/.contributing/bp15_ex2.jpg)  |
| Testing / Validation / Reproducibility (user-tested) | ✅ | The same `nb_validate.py` script used for local self-testing is also available for GitHub CI user testing. Any user can manually run the tests in our repo's GitHub Actions, see the job log's console output, and download the nb-validate artifact to view the results. | 1) GitHub CI job results:![alt text](.alt-text-images-for-docs/.contributing/bp16_ex1.jpg)</br></br>2) nb-validate artifact link:![alt text](.alt-text-images-for-docs/.contributing/bp16_ex2.jpg)</br></br>3) Downloaded artifact:![alt text](.alt-text-images-for-docs/.contributing/bp16_ex3.jpg) |
