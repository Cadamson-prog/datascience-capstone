...
Fourteen features engineered from elemental composition to improve GSR discrimination. The "Baseline" columns indicate whether the feature was included in each model's training set (✓ = included, blank = excluded).
...
| `log_pb_plus_sb` | float | ✓ | ✓ | | 0.9973 | log(1 + Pb + Sb). Additive log-compressed quantification of the core GSR element pair, handling particles where only one of Pb or Sb is present (as opposed to the multiplicative `pb_times_sb`). Dropped in v3 to test whether raw elemental features alone were sufficient. |
| `pb_sb_over_non_ba_mass` | float | ✓ | ✓ | | 0.9979 | (Pb + Sb) as a fraction of total elemental mass excluding Ba. Isolates the Pb-Sb pair to distinguish GSR from barium-heavy Non-GSR confounders (BaAl, BaCaSi). |
| `pb_sb_over_non_ba_o_mass` | float | ✓ | ✓ | | 0.9988 | (Pb + Sb) as a fraction of total elemental mass excluding Ba and oxygen. Highest single-feature PR-AUC; dropped in v3 alongside `pb_sb_over_non_ba_mass` due to near-redundancy. |
| `pb_ba_over_non_sb_mass` | float | ✓ | ✓ | ✓ | 0.6925 | (Pb + Ba) as a fraction of total elemental mass excluding Sb. Measures the Pb-Ba pair's dominance over overall particle composition. |
| `pb_ba_over_non_sb_o_mass` | float | ✓ | ✓ | ✓ | 0.7736 | (Pb + Ba) as a fraction of total elemental mass excluding Sb and oxygen. Tests whether removing oxygen dilution improves the Pb-Ba compositional signal. |
| `ba_sb_over_non_pb_mass` | float | ✓ | ✓ | ✓ | 0.6172 | (Ba + Sb) as a fraction of total elemental mass excluding Pb. Captures the Ba-Sb pair signal for BaSb-type GSR particles. |
| `ba_sb_over_non_pb_o_mass` | float | ✓ | ✓ | ✓ | 0.6914 | (Ba + Sb) as a fraction of total elemental mass excluding Pb and oxygen. Oxygen-excluded variant of `ba_sb_over_non_pb_mass`. |
| `gsr_over_confounders` | float | ✓ | ✓ | ✓ | 0.9430 | (Pb + Sb) / (Ca + Si + Al + Fe + Ti + Zn + Cu). Ratio of GSR signal to common environmental and mineral confounders, excluding Ba due to its overlap in both GSR and Non-GSR classes. Uses a sentinel value of −1 when the denominator = 0, allowing XGBoost to distinguish undefined ratios from reliable positive ones. |
...
| `cu_zn_over_non_o_mass` | float | ✓ | ✓ | ✓ | 0.4298 | (Cu + Zn) as a fraction of total elemental mass excluding oxygen. Oxygen-excluded variant of `cu_zn_over_mass`. |
...
| `ti_zn_over_non_o_mass` | float | ✓ | ✓ | ✓ | 0.4004 | (Ti + Zn) as a fraction of total elemental mass excluding oxygen. Oxygen-excluded variant of `ti_zn_over_mass`. |
