...
nfi_particle_data = p.merge(
    stub_df,
    how="left",
    left_on="stub_id",
    right_on="id",
    suffixes=("_particle", "_stub"),
)

# stub_df.id = stub_source_df.stub_id
stub_source_unique = stub_source_df.drop_duplicates(subset="stub_id", keep="first")
nfi_particle_data = nfi_particle_data.merge(
    stub_source_unique, how="left", on="stub_id"
)

# source_df.id = stub_source_df.source_id
nfi_particle_data = nfi_particle_data.merge(
    source_df, how="left", left_on="source_id", right_on="id", suffixes=("", "_source")
)
nfi_particle_data.shape

# Save final DF as a Parquet file for efficient storage and retrieval
nfi_particle_data.to_parquet(
    "data/nfi_particle_data_full.parquet", index=False, engine="fastparquet"
)
